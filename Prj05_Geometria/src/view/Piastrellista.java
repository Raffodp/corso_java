		...
		if(formaStanza.equals("t")) {
			...
			if(formaStanza.equals("q")) {
				...
				if(formaStanza.equals("r")) {
					...
					int latoLungo = input.nextInt();
					input.nextLine();
					
					System.out.println("Dimensione lato corto in cm. ");
					int latoCorto = input.nextInt();
					input.nextLine();
					
					Rettangolo r = new Rettangolo(new Segmento(latoCorto), new Segmento(latoLungo));
					stanze[i] = r;
