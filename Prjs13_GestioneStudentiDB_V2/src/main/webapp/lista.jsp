...
<body>
	...
	<a href="aggiungiStudente.jsp">Aggiungi studente</a>
	<table>
		<tr>
			<th>ID</th>
			<th>Nome</th>
		...
		if (lista != null) {
			for (Studente p : lista) {
				...
				out.print("<td>" + "<form method = \"post\" action=\"cancellaStudente?deleteId=" + p.getId() + "\">"
				+ "<input type=\"submit\">Elimina</input>" + "</form> </td>");
				out.print("<td>" + "<form method = \"get\" action=\"modificaStudente\">"
				+ "<input type=\"text\" hidden name=\"modificaId\" value=\"" + p.getId() + "\" >"
				+ "<input type=\"submit\">Modifica</input>" + "</form> </td>");
				out.print("</tr>");
			}
		} else {
